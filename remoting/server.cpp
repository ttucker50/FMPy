...
public:
	...
	FMU(const string &libraryPath) : srv(rpc::constants::DEFAULT_PORT) {
		...
		srv.bind("fmi2SetupExperiment", [this](int toleranceDefined, double tolerance, double startTime, int stopTimeDefined, double stopTime) {
			resetExitTimer();
			int status = FMI2SetupExperiment(m_instance, toleranceDefined, tolerance, startTime, stopTimeDefined, stopTime);
			return createReturnValue(status);
		});
		
		srv.bind("fmi2EnterInitializationMode", [this]() {
			resetExitTimer();
			int status = FMI2EnterInitializationMode(m_instance);
			return createReturnValue(status);
		});

		srv.bind("fmi2ExitInitializationMode",  [this]() {
			resetExitTimer();
			int status = FMI2ExitInitializationMode(m_instance);
			return createReturnValue(status);
		});
		
		srv.bind("fmi2Terminate", [this]() {
			resetExitTimer();
			int status = FMI2Terminate(m_instance);
			return createReturnValue(status);
		});

		srv.bind("fmi2Reset", [this]() {
			resetExitTimer();
			int status = FMI2Reset(m_instance);
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2GetReal", [this](const vector<unsigned int> &vr) {
			...
			int status = FMI2GetReal(m_instance, vr.data(), vr.size(), value.data());
		...
		srv.bind("fmi2GetInteger", [this](const vector<unsigned int> &vr) {
			...
			int status = FMI2GetInteger(m_instance, vr.data(), vr.size(), value.data());
		...
		srv.bind("fmi2GetBoolean", [this](const vector<unsigned int> &vr) {
			resetExitTimer();
			vector<int> value(vr.size());
			int status = FMI2GetBoolean(m_instance, vr.data(), vr.size(), value.data());
			return createIntegerReturnValue(status, value);
		});

		srv.bind("fmi2SetReal", [this](const vector<unsigned int> &vr, const vector<double> &value) {
			resetExitTimer();
			int status = FMI2SetReal(m_instance, vr.data(), vr.size(), value.data());
			return createReturnValue(status);
		});

		srv.bind("fmi2SetInteger", [this](const vector<unsigned int> &vr, const vector<int> &value) {
			resetExitTimer();
			int status = FMI2SetInteger(m_instance, vr.data(), vr.size(), value.data());
			return createReturnValue(status);
		});

		srv.bind("fmi2SetBoolean", [this](const vector<unsigned int> &vr, const vector<int> &value) {
			resetExitTimer();
			int status = FMI2SetBoolean(m_instance, vr.data(), vr.size(), value.data());
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2GetDirectionalDerivative", [this](const vector<unsigned int> &vUnknown_ref, const vector<unsigned int> &vKnown_ref, const vector<double> &dvKnown) {
			...
			int status = FMI2GetDirectionalDerivative(m_instance, vUnknown_ref.data(), vUnknown_ref.size(),
		...
		****************************************************/
		...
		srv.bind("fmi2EnterEventMode", [this]() {
			resetExitTimer();
			int status = FMI2EnterEventMode(m_instance);
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2NewDiscreteStates", [this]() {
			...
			int status = FMI2NewDiscreteStates(m_instance, &eventInfo);
			return createEventInfoReturnValue(status, &eventInfo);
		});

		srv.bind("fmi2EnterContinuousTimeMode", [this]() {
			resetExitTimer();
			int status = FMI2EnterContinuousTimeMode(m_instance);
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2CompletedIntegratorStep", [this](int noSetFMUStatePriorToCurrentPoint) {
			...
			int status = FMI2CompletedIntegratorStep(m_instance, noSetFMUStatePriorToCurrentPoint, enterEventMode, terminateSimulation);
			return createIntegerReturnValue(status, value);
		});
		...
		srv.bind("fmi2SetTime", [this](double time) {
			resetExitTimer();
			int status = FMI2SetTime(m_instance, time);
			return createReturnValue(status);
		});

		srv.bind("fmi2SetContinuousStates", [this](const vector<double> &x) {
			resetExitTimer();
			int status = FMI2SetContinuousStates(m_instance, x.data(), x.size());
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2GetDerivatives", [this](size_t nx) {
			...
			int status = FMI2GetDerivatives(m_instance, derivatives.data(), nx);
		...
		srv.bind("fmi2GetEventIndicators", [this](size_t ni) {
			...
			int status = FMI2GetEventIndicators(m_instance, eventIndicators.data(), ni);
		...
		srv.bind("fmi2GetContinuousStates", [this](size_t nx) {
			resetExitTimer();
			vector<double> x(nx);
			int status = FMI2GetContinuousStates(m_instance, x.data(), nx);
		...
		srv.bind("fmi2GetNominalsOfContinuousStates", [this](size_t nx) {
			...
			int status = FMI2GetNominalsOfContinuousStates(m_instance, x_nominal.data(), nx);
		...
		****************************************************/
		...
		srv.bind("fmi2SetRealInputDerivatives", [this](const vector<unsigned int> &vr, const vector<int> &order, const vector<double> &value) {
			resetExitTimer();
			int status = FMI2SetRealInputDerivatives(m_instance, vr.data(), vr.size(), order.data(), value.data());
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2GetRealOutputDerivatives", [this](const vector<unsigned int> &vr, const vector<int> &order) {
			resetExitTimer();
			vector<double> value(vr.size());
			int status = FMI2GetRealOutputDerivatives(m_instance, vr.data(), vr.size(), order.data(), value.data());
			return createRealReturnValue(status, value);
		});
		...
		srv.bind("fmi2DoStep", [this](double currentCommunicationPoint, double communicationStepSize, int noSetFMUStatePriorToCurrentPoint) {
			resetExitTimer();
			int status = FMI2DoStep(m_instance, currentCommunicationPoint, communicationStepSize, noSetFMUStatePriorToCurrentPoint);
			return createReturnValue(status);
		});
		
		srv.bind("fmi2CancelStep", [this]() {
			resetExitTimer();
			int status = FMI2CancelStep(m_instance);
			return createReturnValue(status);
		});
		...
		srv.bind("fmi2GetStatus", [this](int s) {
			resetExitTimer();
			vector<int> value(1);
			int status = FMI2GetStatus(m_instance, fmi2StatusKind(s), reinterpret_cast<fmi2Status *>(value.data()));
			return createIntegerReturnValue(status, value);
		});
		...
		srv.bind("fmi2GetRealStatus", [this](int s) {
			...
			int status = FMI2GetRealStatus(m_instance, fmi2StatusKind(s), value.data());
			return createRealReturnValue(status, value);
		});
		...
		srv.bind("fmi2GetIntegerStatus", [this](int s) {
			resetExitTimer();
			vector<int> value(1);
			int status = FMI2GetIntegerStatus(m_instance, fmi2StatusKind(s), value.data());
			return createIntegerReturnValue(status, value);
		});
		...
		srv.bind("fmi2GetBooleanStatus", [this](int s) {
			resetExitTimer();
			vector<int> value(1);
			int status = FMI2GetBooleanStatus(m_instance, fmi2StatusKind(s), value.data());
			return createIntegerReturnValue(status, value);
		});
